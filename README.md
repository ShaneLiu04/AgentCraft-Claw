...
- **智能问答（Chat）** — 基于七步检索链（Vector + BM25 + RRF + Rerank + Compression）的精准 RAG，LLM 流式生成采用完全异步订阅（无 `blockLast()` 阻塞 Tomcat 线程），支持 SSE 流式输出、来源可溯与答案句子级溯源
...
- **数据大屏** — 基于真实 qa_message 数据的总览、趋势、工具分布、知识库热度、兜底率与质量统计；后端内置 Micrometer 可观测指标
...
1. **MySQL 初始化**：首次启动会自动执行 `docs/agentcraft.sql` 初始化数据库，包含所有表结构和初始数据（含 `sys_ai_config` 默认配置）
